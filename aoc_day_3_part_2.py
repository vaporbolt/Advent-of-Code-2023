...
def populate_viable_parts(grid):
    ...
    for i in range(len(grid)):
        for j in range(len(grid[0])):
            symbol = grid[i][j]
            # if it's a symbol, we gotta get all adjacent numbers, and mark those numbers with "" for "Taken"
            if symbol == "*":
                adjacents = []
                copy_grid = grid # create copy because we only don't want to include duplicates in a  singular * check
                ...
                adjacents.append(check_number(grid, i -1, j + 1)) # check bottom right
                # add the product only if the size == 2
                number_parts = []
                for part in adjacents:
                    if part is not None:
                        number_parts.append(part)
                if len(number_parts) == 2:
                    viable_parts.append(number_parts[0] * number_parts[1])        
